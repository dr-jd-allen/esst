...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/kallen3/Desktop/researchProjects/N-ACT/Measures/Behavioral/Assessment/emotionalStopSignalTask/esst/ESST v2.0.0.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    'Prepare a dictionary to hold the indices for selecting the next stimulus of each type'
    next_stim = {'ambiguous_image': 0, 'negative_image': 0, 'positive_image': 0}
    
    # Initialize go_nogo with a default value or based on experimental conditions
    go_nogo = None  # Example: Set a default value of 'go'
    ...
    for thisPracBlock_loop in pracBlock_loop:
        ...
        for thisPracTrials_loop in pracTrials_loop:
            ...
            # Assuming 'trial_type', 'go_nogo', and 'stim_type' are set from your trial logic
            ...
            # codeTrial Component - End Routine
            ...
            if debug_mode:
                # Simulate a response
                simulated_key = random.choice(['a', 'l'])  # Randomly choose 'a' or 'l'
                simulated_rt = random.uniform(0.3, 0.7)  # Simulate a reaction time between 300ms and 700ms
            
                # Record the simulated response and RT
                keyResp.keys = [simulated_key]
                keyResp.rt = [simulated_rt]
                keyResp.corr = int(simulated_key in [corr_resp1, corr_resp2])  # Correct if simulated key is one of the correct responses
            
                # Add simulated data to the experiment handler
                thisExp.addData('keyResp.keys', simulated_key)
                thisExp.addData('keyResp.rt', simulated_rt)
                thisExp.addData('keyResp.corr', keyResp.corr)
            
                # End the routine early in debug mode
                continueRoutine = False
            else:
                # Check for a response
                if not keyResp.keys:  # No response was made
                    if go_nogo == "go":
                        keyResp.corr = 0  # Incorrect omission error
                        feedback_message = "Miss! Please try to respond more quickly."
                        thisExp.addData('go_accuracy', 0)
                        go_accuracies.append(0)
                    else:
                        keyResp.corr = 1  # Correct non-response
                        feedback_message = "Correct! No response needed on stop trials."
                        thisExp.addData('no_go_acc', 1)
                        stop_accuracies.append(1)
                else:
                    # A response was made
                    if go_nogo == "go":
                        ...
                        # This was a no-go/stop-signal trial
                        keyResp.corr = 0  # Any key press is incorrect
                        thisExp.addData('no_go_acc', 0)
                        stop_accuracies.append(0)
                ...
                # Log the reaction time for go trials
                if go_nogo == "go":
                    thisExp.addData('go_reaction_time', keyResp.rt)
                    if keyResp.rt is not None:
                        go_reaction_times.append(keyResp.rt)
                
                # Adjust buzz_time for no-go trials and keep within bounds
                if go_nogo != "go" and not keyResp.keys:
                    buzz_time = min(buzz_max, buzz_time + 0.05)
                    buzz_timers[stim_type] = buzz_time
                    thisExp.addData('new_buzz_time', buzz_time)
                elif go_nogo != "go" and keyResp.keys:
                    buzz_time = max(buzz_min, buzz_time - 0.05)
                    buzz_timers[stim_type] = buzz_time
                    thisExp.addData('new_buzz_time', buzz_time)
            
                # Ensure buzz_time stays within bounds
                buzz_time = max(min(buzz_time, buzz_max), buzz_min)
                buzz_timers[stim_type] = buzz_time
                thisExp.addData('adjusted_buzz_time', buzz_time)
